...
import { OptimizePromptRequest } from 'genai-web';
  ...
  execute: async function* (props: OptimizePromptRequest) {
    ...
    try {
      ...
      if (response.optimizedPrompt) {
        for await (const event of response.optimizedPrompt) {
          ...
          if (event.optimizedPromptEvent?.optimizedPrompt?.textPrompt?.text) {
            const chunk = event.optimizedPromptEvent?.optimizedPrompt?.textPrompt?.text;
            yield chunk;
          }
      ...
      if (e instanceof ThrottlingException || e instanceof ServiceQuotaExceededException) {
        ...
        yield JSON.stringify('ただいまアクセスが集中しているため時間をおいて試してみてください。');
      } else if (e instanceof ValidationException) {
        yield JSON.stringify(`利用上限に達したか不正なリクエストです \n ${e}`);
      } else {
        console.error(e);
        yield JSON.stringify('エラーが発生しました。時間をおいて試してみてください。');
      }
    }
  },
};
