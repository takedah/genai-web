...
import { AgentMap, ApiInterface, BraveSearchResult, Model, UnrecordedMessage } from 'genai-web';
...
const getAgentInfo = async (agentId: string, agentAliasId: string) => {
  ...
  if (!agentInfoMap[agentAliasId]) {
    ...
    const agentVersion =
      agentAliasInfoRes.agentAlias?.routingConfiguration?.pop()?.agentVersion ?? '1';
    // List Action Group
  ...
  invokeStream: async function* (model: Model, messages: UnrecordedMessage[]) {
    try {
      ...
      const command = new InvokeAgentCommand({
        sessionState: {
          files:
            messages[messages.length - 1].extraData?.map((file, index) => ({
              name: `${toSafeDocumentName(file.name)}-${index}`,
              source: {
      ...
      for await (const streamChunk of res.completion) {
        ...
        if (streamChunk.chunk) {
          ...
          for (const citation of streamChunk.chunk?.attribution?.citations || []) {
            for (const ref of citation.retrievedReferences || []) {
              ...
              if (!s3Uri) continue;
              const url = convertS3UriToUrl(s3Uri, process.env.MODEL_REGION || '');

              // ページ番号を取得
              const pageNumber = ref?.metadata?.['x-amz-bedrock-kb-document-page-number'];
              ...
              // 文中に Reference 追加
              const position =
                (citation.generatedResponsePart?.textResponsePart?.span?.end || 0) + offset + 1;
        ...
        if (streamChunk.trace && streamChunk.trace.trace?.orchestrationTrace) {
          let trace: string = '';
          const rationale = streamChunk.trace.trace?.orchestrationTrace.rationale;
          const invocationInput = streamChunk.trace.trace?.orchestrationTrace.invocationInput;
          const observation = streamChunk.trace.trace?.orchestrationTrace.observation;
          ...
          if (rationale?.text) {
            ...
            if (invocationInput.codeInterpreterInvocationInput?.code) {
              ...
              trace = '```python' + invocationInput.codeInterpreterInvocationInput.code + '\n```';
            } else if (invocationInput.actionGroupInvocationInput?.actionGroupName) {
              // カスタムアクション
              // 自前のアクションを呼び出す時は必要に応じてここを編集
              if (invocationInput.actionGroupInvocationInput.actionGroupName === 'Search') {
                // 検索エージェントは検索キーワードを表示
                const content =
                  invocationInput.actionGroupInvocationInput.requestBody?.content || {};
                const parameters: Parameter[] | undefined = content['application/json'];
                trace =
                ...
                trace = invocationInput.actionGroupInvocationInput.actionGroupName;
              }
            } else if (invocationInput.knowledgeBaseLookupInput?.text) {
              // Knowledge Base は検索キーワードを表示
              trace = 'Search: ' + invocationInput.knowledgeBaseLookupInput.text;
            }
          } else if (observation) {
            // Action からの出力
            if (observation.codeInterpreterInvocationOutput?.executionOutput) {
              // CodeInterpreter の出力（Python の stdout）はそのまま表示
              trace = observation.codeInterpreterInvocationOutput.executionOutput;
              ...
              if (output.startsWith('<search_results>')) {
                ...
                const searchResult: BraveSearchResult[] = JSON.parse(
                  output.replace('<search_results>', '').replace('</search_results>', ''),
                );
                trace = searchResult.map((item) => `- [${item.title}](${item.url})`).join('\n');
              } else {
                // それ以外は出力の冒頭1000文字を表示
                trace = output.length > 1000 ? output.slice(0, 1000) + '...' : output;
              }
            } else if (observation.knowledgeBaseLookupOutput?.retrievedReferences) {
              // Knowledge Base はソース URL を表示
              const refs = observation.knowledgeBaseLookupOutput.retrievedReferences?.flatMap(
                (ref) => {
                  ...
                  if (location) {
                    const url = location.uri
                      ? convertS3UriToUrl(location.uri, process.env.MODEL_REGION || '')
                      : location.url;
                    const fileName = url.split('/').pop() || '';
                    const encodedFileName = encodeUrlString(fileName);
                    const pageNumber = ref?.metadata?.['x-amz-bedrock-kb-document-page-number'];
