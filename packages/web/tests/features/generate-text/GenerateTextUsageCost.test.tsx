...
import { GenerateTextUsageCost } from '../../../src/features/generate-text/invoke/components/GenerateTextUsageCost';
...
describe('GenerateTextUsageCost', () => {
  it('トークンとコスト（円）を表示する', () => {
    const { container } = render(
      <GenerateTextUsageCost usageCostHistory={[usdJpyEntry(699290, 34336, 0.02, 3.14)]} />,
    );
  ...
  it('estimatedCost が無い場合はトークンのみ表示しコスト文言を出さない', () => {
    const { container } = render(
      <GenerateTextUsageCost usageCostHistory={[tokenOnlyEntry(100, 20)]} />,
    );
    const text = container.textContent ?? '';
  ...
  it('usageCostHistory が空なら何も描画しない', () => {
    const { container } = render(<GenerateTextUsageCost usageCostHistory={[]} />);
    expect(container.firstChild).toBeNull();
  });

  it('usageCostHistory が undefined なら何も描画しない', () => {
    const { container } = render(<GenerateTextUsageCost usageCostHistory={undefined} />);
    expect(container.firstChild).toBeNull();
  });
});
