...
import { TeamAppCreateButton } from '../../../../src/features/team-apps/components/TeamAppCreateButton';
import { useFetchLoginUser } from '../../../../src/features/team-apps/hooks/useFetchLoginUser';

// Mock dependencies
vi.mock('@/features/team-apps/hooks/useFetchLoginUser');

describe('TeamAppCreateButton', () => {
  const renderWithRouter = (teamId = 'team-1') => {
    return render(
      <MemoryRouter initialEntries={[`/teams/${teamId}/apps`]}>
        <Routes>
          <Route path='/teams/:teamId/apps' element={<TeamAppCreateButton />} />
        </Routes>
      </MemoryRouter>,
    );
  };
  ...
  describe('Rendering', () => {
    it('renders button when user is in SystemAdminGroup and is admin', () => {
      ...
      const link = screen.getByRole('link', { name: 'AIアプリを作成' });
    ...
    it('does not render button when isSystemAdminGroup is false and loginUser is undefined', () => {
      ...
      const link = screen.queryByRole('link', { name: 'AIアプリを作成' });
    ...
    it('renders with correct link text', () => {
      ...
      const link = screen.getByRole('link', { name: 'AIアプリを作成' });
      expect(link.textContent).toBe('AIアプリを作成');
    });
    ...
    it('links to correct URL with teamId', () => {
      ...
      const link = screen.getByRole('link', { name: 'AIアプリを作成' });
      expect(link.getAttribute('href')).toBe('/teams/team-123/apps/create');
    });
  });
});
