...
import { OverflowShadow } from '@/components/ui/OverflowShadow';
...
const PreRenderer = ({ children, node, ...rest }: ComponentProps<'pre'> & ExtraProps) => {
  ...
  return <CodeBlockContainer codeText={codeText} language={language} />;
};

const CodeBlockContainer = ({ codeText, language }: { codeText: string; language?: string }) => {
  const targetRef = useRef<HTMLDivElement>(null);
  return (
    <div className='my-4 border border-solid-gray-420 bg-white rounded-12'>
      {language && (
        <div className='px-4 py-3 font-bold border-b border-solid-gray-420'>{language}</div>
      )}
      <div className='px-4 pt-4 pb-2 [&_pre]:bg-transparent!'>
        <div ref={targetRef}>
          <CodeBlock lang={(language ?? 'text') as BundledLanguage}>{codeText}</CodeBlock>
        </div>
