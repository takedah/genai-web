...
- **ダイアグラム生成の PNG ダウンロードが失敗する。** `packages/web/src/features/generate-diagram/invoke/components/Mermaid.tsx` が `htmlLabels: true` で mermaid を初期化しているため、生成される SVG にノードのラベルが `<foreignObject>`（XHTML）として埋め込まれる。一方 `DownloadButton.tsx` の PNG 出力は SVG を `data:` URI にして `<img>` として読み込み canvas に描画する実装だが、ブラウザは `<img>` 経由の SVG を「secure static mode」で扱い **`foreignObject` を描画しない**。結果として `img.onerror` が発火し、Console に `PNG出力エラー:` と出て失敗する（`reject` に渡るのが `Error` ではなく `Event` オブジェクトのため、Console では原因が読み取りにくい）。SVG ダウンロードは `<img>` を経由しないため正常に動作する。**閉域構成に起因する問題ではなく、ラベル付きのフローチャートであれば upstream の環境でも同様に失敗する。**`packages/web` は差分ゼロを維持する方針のためフォークでは修正しない（2026-09-13 判断）。利用者には SVG でのダウンロードを案内する
