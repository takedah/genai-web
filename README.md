...
# AI インターフェース（ガバメント AI 源内フォーク）

本ソフトウェアは、デジタル庁が公開する [ガバメント AI 源内](https://www.digital.go.jp/policies/genai) を基に開発された派生物です。**デジタル庁による推奨、認証又は保証を受けたものではありません。**

## ライセンスとブランド要素

本プロジェクトは MIT License のもとに公開されています。詳細は LICENSE ファイルを参照のこと。なお、コードのライセンス許諾は、公式ロゴ等のブランド要素の使用許諾を含まない。
...
- [事前準備](./docs/事前準備.md) 及び [デプロイ手順](./docs/デプロイ手順.md) は、閉域ネットワーク環境で動作するように内容の一部を改変しています。
- [カスタムドメイン設定](./docs/カスタムドメイン設定.md) 及び [SAML 認証手順](./docs/SAML認証手順.md) はこの改変したプロジェクトでは使えない機能となっています。

以下は、フォーク元の README.md の記載です。
